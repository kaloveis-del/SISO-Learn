...
- [ ] 10. Task 10: 에러 처리 및 오프라인 대응 구현
  ...
  - [ ] 10.2 쿠키 에러 메시지 연동 (에러 발생 시 쿠키가 위로(🥺) 감정으로 에러 안내)
  - [ ] 10.3 오프라인 감지 및 UI 처리 (NetworkMonitor → 쿠키 오프라인 메시지 + 배너)
  - [ ] 10.4 API 분당 한도 초과 처리 (60초 대기 타이머 + 쿠키 안내 메시지)
  - [ ] 10.5 일일 한도 초과 처리 (학습 중단 + 쿠키 내일 만나요 메시지)
  - [ ] 10.6 CoreData 저장 실패 처리 (재시도 버튼 + 쿠키 안내)
  - [ ] 10.7 에러 처리 단위 테스트 작성 (`ErrorHandlingTests.swift`)
